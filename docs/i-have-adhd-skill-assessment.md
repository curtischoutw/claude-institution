...
- **輸出風格屬「每次都要守」的常載範疇** → 對應層 1（`hard-rules #14`）＋層 2 情境載入檔（`reporting.md`，
...
新增規則不得凌駕本專案既有的更高優先條文（衝突優先序：誠實條款 > hard-rules > 情境載入檔/skills，見 `CLAUDE.md`）：
...
**結論**：3 條新增規則與既有制度**無不可解衝突**，但導入時必須明寫上述三條邊界，否則會鬆動誠實條款。
...
**建議落點：`reporting.md` 新增一節**（因三條都是「回報寫法」，與 reporting.md 主題一致，屬層 2 情境載入檔）：
...
- 2026-08-26：reporting.md 重組後 §5「ADHD 友善補充」整節溶解——按來源分節改為按功能分節，
  三條規則各歸其位（可見進度 → §1 開頭、一個下一步 → §3 結尾、具體估計 → §4 通篇）。
  第 3 節逐條映射表的節號多數仍成立（§1 結論先行、§2 選擇性省略、§4 長度格式皆原位），
  **僅規則 3 該列的「無直接對應」已過期**——「一個下一步」現與選項表同住 §3，成為該節的
  兩個分支。判定結論（重疊 6／部分 1／新增 3）不變。同次另從本 skill 再蒸餾兩項，
  修正第 3 節規則 10 判「重疊」判得過寬：
  禁用語**具名清單**（§1、§3）與**送出前檢查**（§5）——原判定重疊的是原則，不是可執行度。
  規則 9「清單上限 5」維持不收，理由不變（§3 選項 ≤3、§4 ≤10 行兩個更嚴的上限已管同一件事）。
