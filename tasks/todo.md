...
但該好例已於 2026-08-24 的制度精簡（commit `55dafdb`）連同 §1 的正例／反例一起刪除。
實測：`git show 7def344:institution/rules-lib/reporting.md` 有這行，`55dafdb` 起無。

兩件事待決定，**都不在本次重組範圍內，未動**：

1. 該引用是 2026-08-24 起就過期的死引用，與本次重組無關。要不要修 docs。
2. 更值得問的是**要不要把好例加回 §1**。`judgment.md` 每個 rubric 都有正例／反例、
   `reporting.md §3` 保留了選項表好例，唯獨 §1「結論先行」——全檔最核心的一條——現在
   只有抽象判準沒有範例。當時的刪除理由是精簡，但精簡門檻（`rules-lib/` 單檔 200 行）
   離現在的 63 行還很遠，省這 2 行換掉最該有範例的一條，划不划算值得重評。
