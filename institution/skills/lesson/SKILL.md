...
- **單純事實／偏好／build 指令**（例如「這個專案用 pnpm」「使用者偏好簡短回報」）
  → 交給 Claude Code 內建 auto memory，不進本流程（預設開啟、自動記錄，不需要
  人工步驟）。
- **需要變成「以後都要遵守」的規則**（本 skill 的核心用途）→ 走下面 A／B 節。
...
- 2026-08-06：新增「0. 先分流」——事實/偏好走內建 auto memory，本 skill 只保留
  「需要升級成規則」的部分；A 節範例濃縮。詳見 docs/harness-overlap-2026-08.md
  （使用者核准）。
