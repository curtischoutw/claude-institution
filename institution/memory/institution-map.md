...
`rules-lib/` 6 個情境載入檔；skills/ 3 個；hooks/ 4 個層 0 腳本；agents/ 3 個對抗審查
subagent，內建 `/code-review`／`/simplify`／`/security-review` 優先，需三個獨立
verdict 才派）。
2026-08-24 跟上 Claude Code 2.1.241 的精簡：hard-rules #11 由「一律派 subagent」改條件式
...
XY problem 核心判準搬進 CLAUDE.md 起手式常載（原情境載入檔 `intake.md` 觸發機制實測不可靠）。
